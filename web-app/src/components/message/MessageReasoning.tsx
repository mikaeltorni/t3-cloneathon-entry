...
export const MessageReasoning: React.FC<MessageReasoningProps> = ({
   ...
   * Preserve content even after streaming ends if no final reasoning is available yet
   */
  const streamingReasoningContent = useMemo(() => {
    // Always use reasoning from metadata if available, regardless of streaming state
    const content = message?.metadata?.reasoning || null;
    // Debug: streaming reasoning content
    // eslint-disable-next-line no-console
    console.log(`[MessageReasoning] streamingReasoningContent for messageId=${messageId}:`, content);
    return content;
  }, [message?.metadata?.reasoning, messageId]);
  ...
  const reasoningContent = useMemo(() => {
    ...
    if (streamingReasoningContent) {
      const isCurrentlyStreaming = isStreamingReasoning;
      console.log(`[MessageReasoning] Rendering ${isCurrentlyStreaming ? 'active' : 'preserved'} streaming reasoning for messageId=${messageId}`);
      return (
        <div className="mb-3">
          <ReasoningDisplay 
            reasoning={streamingReasoningContent}
            className={isCurrentlyStreaming ? "animate-pulse" : "opacity-75"} // Pulse if actively streaming, faded if preserved
          />
        </div>
      );
    }
